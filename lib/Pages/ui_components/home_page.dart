...
class _HomepageState extends State<Homepage> {
  ...
  Future<void> checkAndResetRecurringTasks() async {
    try {
      final shouldReset = await firestoreService.shouldResetRecurringTasks();

      if (shouldReset) {
        debugPrint('🔄 Resetting recurring tasks for new day...');
        await firestoreService.resetRecurringTasks();
        await firestoreService.markRecurringTasksReset();
        debugPrint('✅ Recurring tasks reset complete');
      }
    } catch (e) {
  ...
  /// Build stat card for history dialog
  ...
  /// Build mini heatmap (simple grid version)
  Widget _buildMiniHeatmap(Map<String, int> history) {
    final now = DateTime.now();
    final List<Widget> rows = [];

    // Create 13 weeks (91 days)
    for (int week = 0; week < 13; week++) {
      final List<Widget> cells = [];

      for (int day = 0; day < 7; day++) {
        final date = now.subtract(Duration(days: (12 - week) * 7 + (6 - day)));
        final dateKey =
            '${date.year}-${date.month.toString().padLeft(2, '0')}-${date.day.toString().padLeft(2, '0')}';
        final hasCompletion = history.containsKey(dateKey);

        cells.add(
          Tooltip(
            message: hasCompletion ? '$dateKey ✓' : '$dateKey',
            child: Container(
              width: 12,
              height: 12,
  ...
  int _calculateTaskStreak(Map<String, int> completionsByDate) {
    ...
    for (int i = 0; i < 365; i++) {
      ...
      if (completionsByDate.containsKey(dateKey)) {
        ...
        if (streak > 0) break;
        if (i == 0) continue; // Allow today to be skipped
        break;
      }
    }
    return streak;
  ...
  int _calculateLongestStreak(Map<String, int> completionsByDate) {
    if (completionsByDate.isEmpty) return 0;
  ...
  void addTask() async {
    ...
    final timerMinutes = timerController.text.isNotEmpty
        ...
        : null;
    final isRecurring = isRecurringTask; // ✅ NEW: Capture recurring state

    Navigator.of(context).pop();
    control.clear();
    timerController.clear();
    setState(() {
      isRecurringTask = false; // ✅ NEW: Reset toggle
      _isSyncing = true;
    });
    ...
    try {
      final firebaseId = await firestoreService.addTask(
        ...
        taskName,
        timerMinutes,
        isRecurring, // ✅ NEW: Pass recurring parameter
      );
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 14),
                // ✅ UPDATED: Recurring task toggle
                StatefulBuilder(
                  builder: (context, setDialogState) {
                    return Container(
                      decoration: BoxDecoration(
                        color: isRecurringTask
                            ? tierProvider.primaryColor.withValues(alpha: 0.08)
                            : Colors.grey[50],
                        borderRadius: BorderRadius.circular(12),
                        border: Border.all(
                          color: isRecurringTask
  ...
  void updateTask(int index) {
    ...
    setState(() {
      isRecurringTask = task['isRecurring'] ?? false;
    });
                    ...
                    child: Container(
                      ...
                      child: ElevatedButton(
                        onPressed: () async {
                          ...
                          final isRecurring = isRecurringTask;
                ...
                child: _filteredTaskList.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final task = _filteredTaskList[index];
                          return Todolist(
                            firebaseId: task['firebaseId'], // ✅ NEW
                            onViewHistory: task['isRecurring'] == true
                                ? () => _showTaskHistory(
                                    task['firebaseId'],
                                    task['taskName'],
                                  )
