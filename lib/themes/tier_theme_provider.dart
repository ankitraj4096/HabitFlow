...
class TierThemeProvider extends ChangeNotifier {
  ...
  void setCustomTheme(int tierId) {
    // Tier requirements mapping (id -> completedTasks)
    const tierRequirements = [
      1, 5, 10, 25, 50, 100, 250, 500, 1000, 1750, 2500, 4000, 6000, 8000, 10000, 10001
    ];

    if (tierId > 0 && tierId <= tierRequirements.length) {
      final requiredTasks = tierRequirements[tierId - 1];
  ...
  List<Map<String, dynamic>> getAllTiers() {
    ...
    return tierRequirements.map((requiredTasks) {
  ...
  void _setDefaultTheme() {
    _userTier = _firestoreService.getUserTier(1); // Get first tier
    _extractTierTheme();
  }
